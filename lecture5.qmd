...
Answer: 4, 4 and 8 because `increment()` is called by reference, `decrement()` is called by value and `doubling()` takes a pointer and dereferences it to modify the original value.
